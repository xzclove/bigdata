package com.xzc.hadoop.test.server;
...
import com.xzc.hadoop.test.bean.ProductModel;

/**
 * @desc mapreduce操作hbase
 * @author 9256541402qq.com
 * @date 创建时间：2016年6月8日 下午2:45:10
 * @version 1.0.0
 */
public class HBaseTableDemo {
	...
	static class DemoReducer extends TableReducer<Text, ProductModel, ImmutableBytesWritable> {
		@Override
		protected void reduce(Text key, Iterable<ProductModel> values, Context context) throws IOException,
				InterruptedException {
	...
	static Job initFailureLocalHbaseMapReducerJobConfig() throws Exception {
		...
		// 设置mapper相关，mapper从hbase输入
		TableMapReduceUtil
				.initTableMapperJob("data", new Scan(), DemoMapper.class, Text.class, ProductModel.class, job);
